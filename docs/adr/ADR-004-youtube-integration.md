...
- `ffmpeg` がシステムにインストールされている前提（動画ダウンロード後の補助的な動画処理や proxy 作成に使う）

## 追記

- 2026-04-08: ローカル動画の音声抽出は廃止したため、`ffmpeg` は現行の `vod analyze` ローカル分析本体の必須要件ではない。録画手順や proxy 作成などの周辺ワークフローでは引き続き有用。
