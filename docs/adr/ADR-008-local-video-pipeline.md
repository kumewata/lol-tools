...
- 2026-04-08: `extract_audio()` と `transcribe_audio()` は削除した。ローカル動画に対する Commentary 分析とモード自動判定も廃止し、現行実装のローカル動画パイプラインは Gameplay 分析専用になっている。
- 2026-04-08: この ADR は採択時点の設計判断を残すため本文は維持する。現行仕様を確認するときは上記の追記を優先すること。
