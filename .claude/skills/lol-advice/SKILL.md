...
1. ユーザーに Riot ID を確認する（前回のIDを覚えていればそれを提案）
2. 最新データを取得するため、以下を実行する:
   ```bash
   cd lol_review && uv run python -m src.cli report "{RiotID}" --no-open
   ```
3. `lol_review/output/latest_findings.json` を読み込む
...
`lol_review/output/` 内の `findings_*.json` をタイムスタンプ順にリストし、最新と1つ前のスナップショットを比較する:

1. Glob で `lol_review/output/findings_*.json` を取得
