...
以下の手順で active プランを更新または新規作成する:

1. 現在の日付ラベルを算出（例: 2026-05-10）
2. `practice status --json` の `plans[0].date` と比較
3. 一致する場合: 既存プランを保持（追加生成しない）
4. 異なる場合: 新規プランを作成
...
- フォーマット:
  ...
  ---
  date: "{YYYY-MM-DD}"
  generated_at: "{現在 ISO datetime}"
  based_on_snapshot: "{latest_findings.json の snapshot_id 相当}"
  target_summoner: "{.env の DEFAULT_RIOT_ID}"
  status: active
  ---
  ...
  ## 1. {category} / {severity}
  - **元 finding**: {message}
  - **練習ポイント**: {LLM が finding と 8 観点アドバイスから生成する具体的な練習法}
  - **目標**: {LLM が定量化できる目標を生成。できなければ空でよい}
  - **進捗**: pending
  - **note**: 

  ## 2. ...
  ```
- 選定ロジック: 最新 findings の severity 順（critical → warning → info）で全件、category 単位でユニーク化（同 category の重複は最重大 severity を採用）
- 練習ポイントは「8 観点アドバイス」の文脈を踏まえて自然言語で書く（テンプレ的にしない）
